...
// corporate form data view to admin
corporateformRouter.get('/corporatelist', async function (req, res) {
...
corporateformRouter.delete('/remove/:id',async (req,res)=>{
    try{
   id = req.params.id;
   await corporateApplicationData.findById({"_id":id})
   .then((indus)=>{
        indus.remove()   
       res.send(true);
   })  
